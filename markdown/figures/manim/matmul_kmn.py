...
class MatmulKmn(Slide):
    ...
    def construct(self):
        # Tile
        title = Text("Matrix Multiplex, MKN").move_to(UP * (self.N/2 + 2))
        # Grid
        ...
        # Loop M
        left_rect_zero = LEFT * (self.N + self.N / 2 + self.Space - 0.5) + UP * (self.N / 2 - 0.5)
        right_rect_zero = UP * (self.N / 2 - 0.5)
        dest_rect_zero = RIGHT * (self.N + self.Space) + UP * (self.N / 2 - 0.5)

        left_elem = Rectangle(color=BLUE, height=1.0, width=1.0, fill_opacity=0.5) \
            .move_to(left_rect_zero)
        right_elem = Rectangle(color=BLUE, height=1.0, width=self.N, fill_opacity=0.5) \
            .move_to(right_rect_zero)
        ...
        # Loop K
        left_rect_zero = LEFT * (self.N + self.N / 2 + self.Space - 0.5)
        right_rect_zero = UP * (self.N / 2 - 0.5)
        dest_rect_zero = RIGHT * (self.N + self.Space)

        left_elem = Rectangle(color=BLUE, height=self.M, width=1.0, fill_opacity=0.5) \
            .move_to(left_rect_zero)
        right_elem = Rectangle(color=BLUE, height=1.0, width=self.N, fill_opacity=0.5) \
            .move_to(right_rect_zero)
